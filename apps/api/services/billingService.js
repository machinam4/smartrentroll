...
class BillingService {
  ...
  async processPayment(invoiceId, paymentData) {
    try {
      ...
      const penaltyResult = await this.calculatePenalty(invoiceId);
      
      // Update payment reference in invoice
      ...
      // Generate receipt
      const receipt = await this.generateReceipt(payment._id);

      // Log audit
      await this.logAudit('Payment', payment._id, 'CREATE', {
        invoiceId: invoice._id,
        amount: payment.amount,
  ...
  async generateReceipt(paymentId) {
    try {
      ...
      const receiptNumber = utils.generateId('RCP');

      const receipt = new models.Receipt({
        paymentId: payment._id,
        invoiceId: payment.invoiceId._id,
        premiseId: payment.premiseId._id,
        receiptNumber,
        amount: payment.amount,
